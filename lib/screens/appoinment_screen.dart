...
class _AppointmentScreenState extends State<AppointmentScreen> {
  ...
  requestAppointment(courseId, title, List<Appointment> appointment) async {
    ...
    final res = await http.post(Uri.parse(url), headers: {HttpHeaders.authorizationHeader: "Bearer $authToken", "Accept": "application/json"}, body: {"course_id": "$courseId", "title": "$title"});
  ...
  deleteAppointment(id) async {
    ...
    final res = await http.post(
      Uri.parse(url),
      headers: {HttpHeaders.authorizationHeader: "Bearer $authToken", "Accept": "application/json"},
    );
    if (res.statusCode == 200) {
      Fluttertoast.showToast(msg: translate("Appointment_deleted_successfully"), backgroundColor: Colors.green, textColor: Colors.white);
      setState(() {});
    } else {
      Fluttertoast.showToast(msg: translate("Something_went_wrong"), backgroundColor: Colors.red, textColor: Colors.white);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    var appointment = Provider.of<ContentProvider>(context).contentModel != null ? Provider.of<ContentProvider>(context).contentModel.appointment : [];
    return Scaffold(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Text(
                                  DateFormat.yMMMd().add_jm().format(appointment[index].updatedAt),
                                  style: new TextStyle(
                                                ...
                                                child: Column(
                                                  children: [
                                                    Row(
                                                      ...
                                                      children: [
                                                        Text(
                                                          translate("Appointment_Response"),
                                                          style: TextStyle(
                                                            color: mode.titleTextColor,
                                                            fontWeight: FontWeight.w500,
                                                            fontSize: 18.0,
