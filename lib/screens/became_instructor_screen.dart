...
class _BecomeInstructorState extends State<BecomeInstructor> {
  ...
  String _fname = "", _lname = "", _email = "", _phone = "", _detail = "", _age = "";
  TextStyle _labelStyle = TextStyle(fontSize: 17, fontWeight: FontWeight.w600, color: Colors.grey[600]);
  ...
  Widget gender(Color borderClr) {
    return Container(
      ...
      child: DropdownButtonFormField(
        ...
        value: _gender,
        decoration: InputDecoration(labelText: translate("Gender_"), focusedBorder: foborder(borderClr), enabledBorder: enborder(borderClr), floatingLabelBehavior: FloatingLabelBehavior.always, labelStyle: _labelStyle),
      ),
    );
  }
  ...
  Widget inputField(String label, int idx, Color borderclr) {
    return Container(
      ...
      child: TextFormField(
        ...
        keyboardType: idx == 5 || idx == 3 ? TextInputType.number : TextInputType.text,
  ...
  Widget dateofbirth(Color borderClr) {
    return Container(
      ...
      child: TextFormField(
        ...
        readOnly: true,
        style: TextStyle(color: txtColor, fontSize: 17, fontWeight: _datesel ? FontWeight.normal : FontWeight.w600),
        decoration: InputDecoration(
            focusedBorder: foborder(borderClr),
            enabledBorder: enborder(borderClr),
  ...
  Widget imagepickerfield(Color borderClr) {
    return Container(
      ...
      child: TextFormField(
        ...
        style: TextStyle(color: txtColor, fontSize: 17, fontWeight: _imgsel ? FontWeight.normal : FontWeight.w600),
        decoration: InputDecoration(
            suffixIcon: IconButton(
                icon: Icon(
  ...
  Future<bool> sendDetailsForInstructor() async {
    ...
    var _body =
        FormData.fromMap({"fname": _fname, "lname": _lname, "age": _age, "email": _email, "gender": _gender, "mobile": _phone, "detail": _detail, "file": await MultipartFile.fromFile(_image.path, filename: imageName), "image": await MultipartFile.fromFile(_resume.path, filename: resumeName)});
  ...
  Widget resumePicker(Color borderClr) {
    return Container(
      ...
      child: TextFormField(
        ...
        readOnly: true,
        style: TextStyle(color: txtColor, fontSize: 17, fontWeight: _resumeSel ? FontWeight.normal : FontWeight.w600),
        decoration: InputDecoration(
            suffixIcon: IconButton(
                icon: Icon(
                  FontAwesomeIcons.upload,
      ...
      onPressed: () async {
        ...
        if (_formKey.currentState.validate() == true) {
          ...
          if (x) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(translate("Request_sent"))));
          } else {
            Navigator.pop(context);
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(translate("Request_sending_failed"))));
          }
        }
        setState(() {
          submitLoading = false;
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            Expanded(flex: 1, child: inputField(translate("First_Name"), 0, borderClr)),
            SizedBox(
              width: 10.0,
            ),
            Expanded(flex: 1, child: inputField(translate("Last_Name"), 1, borderClr)),
          ],
        ),
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
  ...
  Widget scaffoldView(mode) {
    return Container(
      ...
      child: SingleChildScrollView(child: Form(key: _formKey, child: form(mode.notificationIconColor))),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: secondaryAppBar(mode.notificationIconColor, mode.bgcolor, context, translate("Become_an_Instructor")),
